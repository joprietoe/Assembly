...
int main(int argc, char ** argv) {
	if (argc<3) {
		fprintf(stderr, "usage: assembly [path] [iterations] [memory budget in GB]\n");
		exit(-1);
	}
    ...
    BigVector<VertexId> indegree(graph.path+"/indegree", graph.vertices);
	BigVector<test> degree_test(graph.path+"/degree_test", graph.vertices);
	...
	/*test empty ={0};
	degree_test.fill(empty);

	for(int i = 0; i < graph.vertices; i++){
        test *tempV = &degree_test[i];
		printf("Degree test %d --- %d\n",i, tempV->degree);
    }*/
    ...
    //printf("Vertex out degree\n");

    for(int i = 0; i < graph.vertices; i++){
        printf("%d,%d,%d \n",i, degree[i], indegree[i] );
    }
